...
- (void)viewDidLoad {
    [super viewDidLoad];
//    self.array=[[NSMutableArray alloc]initWithObjects:@"登录",@"收藏列表",@"下载列表",@"最近收听",@"定时功能",@"设置",@"意见与反馈",nil];
//    self.array = self.myarray;
    [self createUI];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.array[index] isEqualToString:@"登录"]) {
    ...
    }else if ([self.array[index] isEqualToString:@"设置"]){
        SettingController *settingVC = [SettingController new];
        [self.navigationController pushViewController:settingVC animated:YES];
    }else if ([self.array[index] isEqualToString:@"意见与反馈"]){
        OpinionController *opinionVC = [OpinionController new];
